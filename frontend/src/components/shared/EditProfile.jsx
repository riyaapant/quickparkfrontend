...
export default function EditProfile() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await api.put(`${config.BASE_URL}/profile`, user);
            ...
            <div className="pt-8 flex-grow flex flex-row">
                ...
                <div className="flex-grow divide-y">
                    ...
                    <div className="px-2 py-3 sm:grid sm:grid-cols-3 sm:gap-2 sm:px-0">
                        ...
                        <dd className='mt-1 text-lg text-gray-800 sm:col-span-2 border-collapse border rounded-xl border-gray-300 px-2 py-3'>
                            <div className='flex flex-row gap-x-2'>
                                <Paperclip className='w-auto cursor-pointer' onClick={handleBrowseClick} />
                                <p className='flex-grow truncate'>{user.document}</p>
                                {user.document ? (
                                    <button className='text-qp hover:text-blue-700 w-auto font-semibold'>
                                        Download
                                    </button>
                                ) : ('Add Document')}
                                <input
